...
class AvalancheDataset(IDatasetWithTargets[T_co, TTargetType], Dataset[T_co]):
    def __init__(self,
        ...
        self.scene_ids: Sequence[Any] = self._initialize_scene_ids_sequence(dataset, scene_ids)
        self.prompts: Sequence[str] = self._initialize_prompts_sequence(dataset, prompts)  # <<< 新增
    ...
    def __getitem__(self, idx):
        try:
            item = TupleTLabel(
                manage_advanced_indexing(idx, self._get_single_item, len(self), self.collate_fn)
            )
        except Exception as e:
            print(f"[EXCEPTION] idx={idx}, dataset size={len(self)}, error={e}")
            raise

        if item is None:
            print(f"[ERROR] manage_advanced_indexing returned None at idx={idx}")
            return None
        ...
        if isinstance(item, tuple):
            try:
                ...
                print(f"[ERROR] scene_ids/prompts out of range: idx={idx}, len={len(self.scene_ids)}")
                raise
            return (*item, sid, prompt)   
        else:
            print("error on avalanchedataset")
            return (item, None, None, self.scene_ids[idx], self.prompts[idx])  

    def __len__(self):
        return len(self._dataset)

    def _initialize_scene_ids_sequence(self, dataset, scene_ids: Optional[Sequence[Any]]) -> Sequence[Any]:
        if scene_ids is not None:
            if len(scene_ids) != len(dataset):
                raise ValueError(
                    f'Invalid number of scene_ids. Got {len(scene_ids)}, expected {len(dataset)}')
            return scene_ids

        return [None] * len(dataset)

    def _initialize_prompts_sequence(self, dataset, prompts: Optional[Sequence[str]]) -> Sequence[str]:
        if prompts is not None:
            if len(prompts) != len(dataset):
                raise ValueError(
                    f'Invalid number of prompts. Got {len(prompts)}, expected {len(dataset)}')
            return prompts
        return [""] * len(dataset)

    def _optimize_prompts(self):
        self.prompts = optimize_sequence(self.prompts)
    ...
    def _process_pattern(self, element: Tuple, idx: int):
        ...
        return TupleTLabel((pattern, label, *element[2:],
                            self.targets_task_labels[idx], scene_id, prompt))

    def _optimize_scene_ids(self):
        self.scene_ids = optimize_sequence(self.scene_ids)
...
class AvalancheSubset(AvalancheDataset[T_co, TTargetType]):
    ...
    def __init__(self,
        ...
        """
        self._scene_ids = scene_ids  # 保存 scene_ids
        ...
        self.scene_ids = self._initialize_scene_ids_sequence(subset, self._scene_ids)
        self.prompts = self._initialize_prompts_sequence(subset, self._prompts)
    '''
    def __getitem__(self, idx):
        item = super().__getitem__(idx)
        if item is None or any(x is None for x in item if isinstance(item, (tuple, list))):
            print(f"[ERROR] Dataset returned None at idx={idx}")
        return item
    '''
    def _initialize_scene_ids_sequence(self, dataset, scene_ids):
        if scene_ids is not None:
            if len(scene_ids) == len(dataset):
                return scene_ids
            elif len(scene_ids) == len(self._original_dataset) and self._indices is not None:
                return [scene_ids[i] for i in self._indices]
            else:
                raise ValueError(f"scene_ids 长度不匹配，dataset={len(dataset)}, scene_ids={len(scene_ids)}")
        return [0] * len(dataset)  # 默认值

    def _initialize_prompts_sequence(self, dataset, prompts):
        if prompts is not None:
            if len(prompts) == len(dataset):
                return prompts
            elif len(prompts) == len(self._original_dataset) and self._indices is not None:
                return [prompts[i] for i in self._indices]
            else:
                raise ValueError(
                    f"prompts 长度不匹配，dataset={len(dataset)}, prompts={len(prompts)}")
        return [""] * len(dataset)  # 默认空字符串

    def scene_ids(self):
        if hasattr(self, '_scene_ids_sequence'):
            return self._scene_ids_sequence
        self._scene_ids_sequence = self._initialize_scene_ids_sequence(
            self._dataset, self._scene_ids)
        return self._scene_ids_sequence

    def prompts(self):
        if hasattr(self, '_prompts_sequence'):
            return self._prompts_sequence
        self._prompts_sequence = self._initialize_prompts_sequence(
            self._dataset, self._prompts)
        return self._prompts_sequence
