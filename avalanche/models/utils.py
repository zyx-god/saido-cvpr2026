...
def avalanche_forward(model, x, task_labels):
    if isinstance(model, MultiTaskModule):
        ...
        return model(x)
'''
def avalanche_forward(model, x, task_labels=None, scene_id=None,batch_prompts=None):
    if isinstance(scene_id, torch.Tensor):
        uniq = torch.unique(scene_id)
        if len(uniq) != 1:
            raise ValueError(f"Batch contains multiple scenes: {uniq}")
        scene_val = int(uniq.item())
    else:
        scene_val = int(scene_id)
    ...
    try:
        return model(x, task_labels, scene_id=scene_id,batch_prompts=batch_prompts)
    except TypeError:
        print("error on avalanche_forward")
        return model(x)
