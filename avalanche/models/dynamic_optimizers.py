...
'''
def reset_optimizer(optimizer, model):
    """Reset the optimizer to update the list of learnable parameters for multiple param groups."""
    optimizer.state = defaultdict(dict)

    # 遍历每个参数组，按名称匹配模型参数
    for group in optimizer.param_groups:
        if 'clip_peft' in group['params'][0].__module__:
            group['params'] = list(model.clip_peft.parameters())
        elif 'visual_projection' in group['params'][0].__module__:
            group['params'] = list(model.visual_projection.parameters())
        elif 'fc' in group['params'][0].__module__:
            group['params'] = list(model.fc.parameters())
        else:
            # 若不匹配模块名，则直接保留原来的参数
