...
class LoadBestPlugin(StrategyPlugin):
    def __init__(self, val_stream_name: str,
        ...
        #self.best_state = torch.load('../clear10_moco_res50_public_private/model/model_RegO_offline_time09.pth')  # Contains the best parameters
    ...
    def before_eval(self,strategy,**kwargs):
        if(self.best_state==None):
            ...
            result = strategy.model.load_state_dict(self.best_state, strict=False)
            print('###########################################################')
            ...
            if result.missing_keys or result.unexpected_keys:
                ...
                if result.missing_keys:
                    print(f'  Missing keys: {len(result.missing_keys)} 个参数')
                    if len(result.missing_keys) <= 3:
                        ...
                        for key in result.missing_keys[:2]:
                            print(f'    - {key}')
                        print(f'    ... 还有 {len(result.missing_keys) - 2} 个参数')
                if result.unexpected_keys:
                    print(f'  Unexpected keys: {len(result.unexpected_keys)} 个参数')
                    if len(result.unexpected_keys) <= 3:
                        ...
                        for key in result.unexpected_keys[:2]:
                            print(f'    - {key}')
                        print(f'    ... 还有 {len(result.unexpected_keys) - 2} 个参数')
            print('###########################################################')
    '''
    def _update_best(self, strategy):
        ...
        if(val_acc==None):
            ...
            print('if val_acc=None,val_acc:',val_acc)
        ...
        '''
    def _update_best(self, strategy):
        # 直接用 base_strategy 中的验证集 acc
        val_acc = getattr(strategy, "_last_val_acc", None)

        #print("val_acc (from base_strategy):", val_acc)

        if val_acc is not None and (
                self.best_val is None or self.operator(val_acc, self.best_val)
        ):
            self.best_state = deepcopy(strategy.model.state_dict())
            self.best_val = val_acc
            self.best_epoch = strategy.clock.train_exp_epochs
