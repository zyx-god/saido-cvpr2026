...
from avalanche.training.strategies.SceneBaseStrategy import SceneBaseStrategy

class SAIDO(SceneBaseStrategy):
    def __init__(self, model: Module, optimizer: Optimizer, criterion,
                 ef_thresh, importnat_thresh,
                 alpha: Union[float, Sequence[float]], temperature: float,
                 train_mb_size: int = 1, train_epochs: int = 1,
                 eval_mb_size: int = None, device=None,
                 plugins: Optional[List[StrategyPlugin]] = None,
        ...
        saido_plugin = SAIDOPlugin(model=model, ef_thresh=ef_thresh, importnat_thresh=importnat_thresh,
                                 criterion=criterion,use_ebbinghaus=False)
        if plugins is None:
            plugins = [saido_plugin]
        else:
            plugins.append(saido_plugin)
